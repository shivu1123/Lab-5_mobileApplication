...
// Stateful widget for Pokémon battle screen
...
class _PokemonBattleScreenState extends State<PokemonBattleScreen> {
  ...
  Future<void> fetchPokemon(String name, int player) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data['data'].isNotEmpty) {
        setState(() {
          if (player == 1) {
            pokemon1 = data['data'][0]; // Store Pokémon 1 details
          } else {
            pokemon2 = data['data'][0]; // Store Pokémon 2 details
          }
        });
      } else {
        showError('Pokémon not found!'); // Show error if no Pokémon found
      }
    } else {
      showError('Failed to load Pokémon'); // Show error on request failure
    }
  }
            ...
            Row(
              ...
              children: [
                pokemon1 != null
                    ...
                    : Container(width: 150, height: 200), // Placeholder
                pokemon2 != null
  ...
  Widget buildPokemonCard(Map<String, dynamic> pokemon) {
    return Column(
      children: [
        Image.network(pokemon['images']['small'], height: 150), // Display Pokémon image
        Text(
          pokemon['name'],
          style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
        ),
        Text("HP: ${pokemon['hp']}"), // Display Pokémon HP
      ],
    );
  }
}